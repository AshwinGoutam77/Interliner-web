...
import { TrackOrdersTranslations } from "@/data";
...
export default function DashboardPage() {
    ...
    const content = TrackOrdersTranslations[lang] || TrackOrdersTranslations.en;
    ...
    const orderColumns = [
        ...(role === "sales"
            ...
            : []),
        { name: content.orderDate, selector: (row) => row.date },
        { name: content.orderNumber, selector: (row) => row.number },
        {
            name: "",
            cell: (row) => (
                <Link
                ...
                >
                    {content.trackOrder} <ChevronRight color="#012169" />
                </Link>
            ),
        },
    ];
    ...
    return (
        <div className="p-6 bg-gray-50 lg:mt-0 md:mt-10 min-h-screen">
            <div className="flex items-center justify-between gap-4 mb-8">
                <h1 className="text-3xl font-bold text-dark">{content.title}</h1>
            ...
            {role === "sales" && isModalOpen && (
                ...
                >
                    ...
                    >
                        <h2 className="text-lg font-bold mb-4">{content.selectCustomer}</h2>
                        <input
                            type="text"
                        ...
                        <button
                            className="primary-btn mt-4 ml-auto"
                            onClick={() => setIsModalOpen(false)}
                        >
                            {content.clearAll}
                        </button>
